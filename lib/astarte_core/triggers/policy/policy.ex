...
defmodule Astarte.Core.Triggers.Policy do
  ...
  def changeset(%Policy{} = policy, params \\ %{}) do
    # TODO check minimum and maximum range of attributes + conditions on name
    policy
    ...
    |> validate_inclusion(:event_ttl, 1..100)
  ...
  defp validate_all_handlers_on_different_errors(changeset) do
    ...
    if all_different_errors(handlers) do
      changeset
    else
  ...
  # TODO Maybe use a computationally better comparison?
  defp all_different_errors(handlers) do
    errs = List.flatten(Enum.map(handlers, &Handler.error_list/1))
    Kernel.length(errs) == Kernel.length(Enum.uniq(errs))
  end
