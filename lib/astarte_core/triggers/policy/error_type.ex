...
defmodule Astarte.Core.Triggers.Policy.ErrorType do
  ...
  def cast(error_type) when is_binary(error_type) do
    EctoMorph.generate_changeset(%{keyword: error_type}, Policy.KeywordError)
    |> Policy.KeywordError.validate()
  ...
  def cast(error_type) when is_list(error_type) do
    EctoMorph.generate_changeset(%{error_codes: error_type}, Policy.RangeError)
    |> Policy.RangeError.validate()
    |> EctoMorph.into_struct()
    |> case do
      {:ok, struct} -> {:ok, struct}
      # returning changeset.errors ensures the error
  ...
  def dump(error_type) when is_binary(error_type) do
    EctoMorph.cast_to_struct(%{keyword: error_type}, Policy.KeywordError)
  end

  def dump(error_type) when is_list(error_type) do
    EctoMorph.cast_to_struct(%{error_codes: error_type}, Policy.RangeError)
  end
